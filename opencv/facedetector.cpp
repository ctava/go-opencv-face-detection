...
void FaceDetector::detectObjects(const cv::Mat &img, cv::CascadeClassifier &cascade, std::vector<cv::Rect> &objects, int scaledWidth, int flags, cv::Size minFeatureSize, float searchScaleFactor, int minNeighbors)
{
    cv::Mat gray;
    if (img.channels() == 3) {
...
void FaceDetector::findLargestObject(const cv::Mat &img, cv::CascadeClassifier &cascade, cv::Rect &largestObject, int scaledWidth)
{
...
void FaceDetector::initCascadeClassifiers()
{
    try {
...
void FaceDetector::detectBothEyes(const cv::Mat &face, cv::Point &leftEye, cv::Point &rightEye)
{
